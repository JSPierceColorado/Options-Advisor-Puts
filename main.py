# ==========================================================
# PutExecutor v1.1 — S&P 500 (CSV) → Alpaca Bear Screener → Tradier Puts
# - Universe: S&P 500 list via CSV mirrors (no lxml). Small fallback if mirrors fail.
# - Screener (bearish only): tighter trend filter to avoid oversold/extended shorts.
# - Options: Tradier chains (greeks flattened). Select PUTs with:
#       delta in [-0.80, -0.60], OI ≥ 200, Vol ≥ 50, spread ≤ 15%
# - Exits: NEW fixed stop-loss (-8% default) and take-profit (+25% default),
#          plus existing inverse chandelier break, -25% off peak option, ≤7 DTE.
...
class S:
    ...
    # NEW: Profit Target & Stop Loss on option premium
...
def bear_ok(df):
    """
    Tighter bearish health check to avoid oversold/extended names:
    - EMA20 < EMA50
    - RSI falling and bounded: 30 ≤ RSI(14) < 50   (avoid deeply oversold)
    - Price not <95% of EMA20 (avoid chasing breakdowns)
    """
    ...
    if price < float(ef.iloc[-1]) * 0.95: return False  # too extended below EMA20
    return True
...
def sp500_symbols() -> List[str]:
    ...
    sess.headers.update({"User-Agent": "Mozilla/5.0 PutExecutor/1.1"})
...
def manage_open_for_ticker(t: str, cnt: Dict[str,int]):
    ...
    for p in picks:
        ...
        # NEW: hard stop loss on option premium
        if mid <= p["entry_option"] * (1 - S.STOP_LOSS):
            reason = "stoploss"

        # NEW: take profit on option premium
        elif mid >= p["entry_option"] * (1 + S.TAKE_PROFIT):
            reason = "target"

        # Existing: inverse chandelier break on underlying (bullish reversal)
        elif atr_val is not None and spot > trail_bear:
            reason = "trail_bear_break"

        # Existing: peak-to-trough drawdown on option premium
        elif mid <= peak*(1 - S.OPT_DD):
            reason = "drawdown"

        # Existing: time stop when close to expiration
        else:
...
def process_candidate_tradier(t: str, cnt: Dict[str,int]):
    ...
    # Prefer expiry nearest to mid of DTE window
...
def screen_symbol_bear(sym: str) -> bool:
    ...
    if DEBUG:
        ...
        print(f"[{'PASS' if passed else 'FAIL'}-BEAR] {sym}: price={price:.2f}, EMA{S.EMA_FAST}={ef:.2f} < EMA{S.EMA_SLOW}={es:.2f}, RSI={r_last:.1f}")
    return passed
